...
async function getArticle(slug: string): Promise<Article | null> {
  ...
  if (USE_MOCK_DATA || !supabase) {
    return (mockNews as unknown as Article[]).find(a => a.slug === slug) || null
  }
...
async function getRelatedArticles(currentId: string, category?: string): Promise<Article[]> {
  ...
  if (USE_MOCK_DATA || !supabase) {
    let related = (mockNews as unknown as Article[]).filter(a => a.id !== currentId)
    if (category) {
      related = related.filter(a => a.category === category)
    }
  ...
  return (
    <div className="flex-grow pb-20">
      ...
      <div className="max-w-[800px] mx-auto px-4 pt-8">
        ...
        {/* Title */}
        <h1 className="text-[40px] leading-[48px] font-bold text-gray-900 mb-6">
          {article.title}
        </h1>
      ...
      {/* Featured Image */}
      <div className="w-full max-w-[1200px] mx-auto px-4 h-[520px] mb-12">
      ...
      <article className="max-w-[800px] mx-auto px-4">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: sanitizeAndFormatHtml(article.body) }}
        />

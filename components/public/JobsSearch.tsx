...
function JobsSearch({ initialJobs, location: initialLocation, query: initialQuery }: JobsSearchProps) {
  ...
  return (
    <form role="search" aria-label="Tìm việc làm" className="w-full max-w-3xl bg-white rounded-full shadow-lg p-2 flex flex-col md:flex-row items-center gap-2 relative z-10 border border-gray-100" onSubmit={(e) => { e.preventDefault(); handleSearch(); }}>
      ...
      <div className="flex items-center px-4 w-full md:w-48 border-b md:border-b-0 md:border-r border-gray-200 py-3 md:py-0">
        <i className="fa-solid fa-location-dot text-gray-400 mr-2" aria-hidden="true"></i>
        <select
          id="job-location"
          value={selectedLocation}
          onChange={(e) => setSelectedLocation(e.target.value)}
          className="w-full bg-transparent border-none text-gray-600 text-sm focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 appearance-none cursor-pointer"
        >
      ...
      <div className="flex-grow flex items-center px-4 w-full py-3 md:py-0">
        <input
          ...
          className="w-full bg-transparent border-none text-sm text-gray-700 placeholder-gray-400 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
